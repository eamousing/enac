...
contains
    ...
    subroutine update_parameters()
        ...
        if (year .gt. 0) then
            ...
            ! Update individual SI parameters with these multipliers  
    ...
    subroutine decis_param()
        !! decis_param calls linkfunc with corrupted values and updates decparam
        !! Sets up decparam according to varenv for the current species, year and season
        !! The call to linkfunct with option 2 will put the result in decparam
        real :: parmult(maxspec,maxfunc,maxfuncparam) !! Parameter multipliers generated by the link-functions
    ...
    subroutine linkfunc(nlink,parmult,kilde,decp)
        ...
        integer, intent(in) :: nlink, kilde
        ! parmult is parameter multipliers generated by the link-functions
        real, intent(out) :: parmult(maxspec,maxfunc,maxfuncparam), decp(maxspec, 3)
    ...
    subroutine linkfunction_1(decp)
        ...
        real, intent(out) :: decp(maxspec,3)
    ...
    subroutine linkfunction_2(condtemp,kilde)
        ...
        c_season = int(link_parameters(species,2,8)) 
    ...
    subroutine mac_pred()
        !! mac_pred calculates a new sigma (effect) as a function of mackerel predation.
        !! The linear parameters are estimated from a 90th quantile regression on recruitment residuals (fitted from Ricker) vs mackerel abundance
